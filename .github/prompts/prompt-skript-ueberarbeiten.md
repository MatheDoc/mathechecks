...
- Prüfe das Skript im Kontext des zugehörigen Lernbereichs und ziehe bei Bedarf `checks.json` sowie benachbarte Moduldateien heran.
...
- Berücksichtige bei jedem Check-Anker die tatsächlichen Quellen mit: Tipps aus `checks.json` sowie das zugehörige Beispiel und die passende Aufgabensammlung, die über `Nummer` und `Sammlung` des Checks identifiziert werden.
...
- Wenn beim Lesen der Tipps in `checks.json` eindeutige Tippfehler oder fachliche Fehler auffallen (z. B. falsche Formel, falsches Schlagwort, Tippfehler in einem Tipp-String), korrigiere sie direkt in `checks.json` — ohne Rückfrage.

## Ausgabe

- Nimm die Änderungen direkt in den betroffenen Dateien vor (`skript.md` und ggf. `checks.json`).
