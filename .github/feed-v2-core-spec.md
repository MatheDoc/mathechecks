## 2026-06-07 Check-Ketten-Priorität

- Die Cursor-Auswahl priorisiert weiterhin zuerst die Dringlichkeitsklasse: `überfällig` vor `fällig` vor `verfügbar`.
- Innerhalb derselben Dringlichkeitsklasse gewinnt bei Check-Schritten die tiefere Stufe der Kette: `kompetenzliste_gate` vor `feynman` vor `recall` vor `training`.
- Die frühere Plan- oder Verfügbarkeitszeit eines `training` ist innerhalb derselben Dringlichkeitsklasse nur noch ein nachrangiges Feinkriterium.
- Diese Bevorzugung gilt für die Check-Kette; `start` und `flashcards` bleiben eigenständige Aktivitätsklassen.
...
function normalizeFeedAttentionSummary(attentionSummary = null, { minimumCount = 0 } = {}) {
  ...
  const computedTotal = overdueCount + dueCount;
  const explicitTotal = Math.max(0, Number(attentionSummary?.totalCount) || 0);
  const totalCount = Math.max(minimumCount, computedTotal, explicitTotal);
  return {
    overdueCount,
    dueCount,
    totalCount,
...
async function refreshFeedLauncher(state) {
  ...
  try {
    ...
    const [attentionSummaryResult, contentMeta] = await Promise.all([
      loadFeedAttentionSummary({ supabase })
    ...
    const resolvedItem = item?.href ? item : fallbackItem;
    const minimumCount = resolvedItem?.href ? 1 : 0;
    const resolvedAttentionSummary = attentionSummaryResult?.ok
      ? normalizeFeedAttentionSummary(attentionSummaryResult.value, { minimumCount })
      : lastFeedLauncherState.attentionSummary
        ? normalizeFeedAttentionSummary(lastFeedLauncherState.attentionSummary, { minimumCount })
        : normalizeFeedAttentionSummary(createEmptyAttentionSummary(), { minimumCount });
    ...
    if (button.dataset.feedRenderToken === renderToken) {
      ...
      const fallbackAttentionSummary = lastFeedLauncherState.attentionSummary
        ? normalizeFeedAttentionSummary(lastFeedLauncherState.attentionSummary, {
          minimumCount: fallbackItem?.href ? 1 : 0,
        })
        : null;
