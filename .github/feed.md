...
Diese Datei beschreibt das fachliche Zielbild für den deterministischen Feed und ist die kanonische Feed-Doku für die Weiterentwicklung.
...
- `selection_reason`: Grund der letzten Cursor-Wahl. Die aktuelle Runtime nutzt `initial`, `auto_pick`, `repeat_request`, `lock_expired` und `invalidated`.
- `G`: Basisabstand für didaktische Übergänge innerhalb einer Check-Kette.
- `G_step`: Der für einen konkreten Übergang gesetzte Abstand. Für didaktische Übergänge entspricht er dem Profilwert `G`.
- `G_gate`: Gate-Abstand für `start -> training`; hier gilt `G_gate = 0`.
- `M`: Überfälligkeitsfaktor. Für didaktische Übergänge gilt fachlich `M = 2` bezogen auf den Abschluss des Vorgängerschritts.
- `L`: Sticky-Lock-Dauer.
...
Der Feed trennt Gate-Übergänge und didaktische Übergänge:
...
- für didaktische Übergänge: `overdue_from = available_from + G_step`, also hier `completed_at + M * G_step` mit `M = 2`
...
- Ein explizites `target_date` ändert nicht stillschweigend die gespeicherte Session-Geschwindigkeit.
...
Die offene Last wird gewichtet nach Schritttyp betrachtet. Ein möglicher Startwert ist:
...
und für explizite Ziele daraus die erforderliche Geschwindigkeit:
...
Das Druckprofil wählt das gemeinsame `G` für neu entstehende didaktische Übergänge:
...
Der Tageswechsel darf hier lazy beim ersten serverseitigen Feed-Read eines neuen Tages verarbeitet werden. Das ist kein normaler Read-Effekt: Der Server schreibt idempotent nur, wenn der letzte Replan vor dem aktuellen Kalendertag in `Europe/Berlin` lag und sich Werte tatsächlich ändern. Weitere Reads desselben Tages verändern `planned_from` nicht.
...
Die Rangbildung für `planned_from` muss deterministisch sein. Der Feed nutzt dafür diese Sortierung:
...
Für den Lock reicht genau ein Zeitpunkt:
...
Explizites Verlassen meint hier nur die Feed-Entscheidung `Nein, zum Dashboard`. Navigation über Sidebar oder freier Modulzugriff hebt den Lock nicht auf; er bleibt bestehen oder läuft regulär ab.
...
Die frühere Plan- oder Verfügbarkeitszeit eines `training` ist innerhalb derselben Prioritätsklasse also nur noch ein nachrangiges Feinkriterium.
...
Der Feed benötigt fachlich mindestens:
...
Das Frontend soll den Feed nicht per Tabellen-CRUD steuern. Die zentralen Operationen sollten serverseitig laufen:
...
1. Diese Feed-Spezifikation fachlich freigeben.
