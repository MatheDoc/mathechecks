(() => {
    ...
    const CALCULATOR_POPUP_STATE_KEY = 'calculator-popup-state-v1';
    ...
    window.focusCalculatorMainInput = focusMainInputForCurrentDevice;
    ...
    function renderLGSMatrix() {
        ...
        for (let equationIndex = 0; equationIndex < state.lgsEquations; equationIndex++) {
            ...
            for (let variableIndex = 0; variableIndex < state.lgsVariables; variableIndex++) {
                ...
                variableLabel.textContent = CalculatorCommands.getLGSVariableName(variableIndex);
    ...
    function renderGraphPanelPreview({ funktionen, optionen, pointsHtml }) {
        ...
        try {
            if (typeof window.renderCalculatorGraph !== 'function' || typeof window.Plotly === 'undefined') {
                throw new Error('graph-renderer-unavailable');
            }
            window.renderCalculatorGraph('graphPanelPreview', funktionen, optionen);
            return true;
        } catch {
            graphPanelPreview.innerHTML = '';
            graphPanelPreview.hidden = true;
    ...
    function runGraphLivePreviewNow({ includeAnalysis = true } = {}) {
        ...
        CalculatorCommands.execute(command, liveGraphOutputApi, { includeGraphAnalysis: includeAnalysis });
    }
    ...
    function getGraphCommandFromPanel() {
        ...
        return CalculatorCommands.buildGraphCommand(fields);
    }
    ...
    const outputApi = {
        ...
        setMatrix(matrix) {
            ...
            if (resultDisplay) {
                ...
                resultDisplay.appendChild(CalculatorCommands.renderMatrixGrid(matrix));
            }
            state.ans = CalculatorCommands.matSerialize(matrix);
        },
    ...
    function syncBinomPreview() {
        ...
        resultTarget.textContent = CalculatorCommands.getLiveBinomResult(fields);
    }

    function syncLGSCommandPreview(options = {}) {
        const values = getLGSValues();
        const command = CalculatorCommands.buildLGSCommand({
        ...
        if (resultTarget) {
            if (!hasAnyValue) {
                ...
                CalculatorCommands.execute(command, {
                    setText(text) {
    ...
    function syncBinomCommandPreview(options = {}) {
        syncBinomPreview();
        if (!options.commitToMainInput) return;
        insertIntoMainInput(CalculatorCommands.buildBinomCommand(getBinomFields()));
    }
    ...
    function createUnaryStandardTool({ key, label = key, focusId, inputId, expressionName = key }) {
        return {
            ...
            build: () => CalculatorCommands.buildUnaryFunctionExpression(expressionName, byId(inputId)?.value || ''),
        };
    }
    ...
    function getStandardToolDefinitions() {
        return {
            ...
            log: {
                ...
                build: () => CalculatorCommands.buildLogExpression({
            ...
            exp: {
                ...
                build: () => CalculatorCommands.buildExpExpression(byId('funcExpValue')?.value || ''),
            },
            factorial: {
                label: 'Fakultät',
                ...
                build: () => CalculatorCommands.buildFactorialExpression(byId('funcFactorialValue')?.value || ''),
            },
            ncr: {
                label: 'nCr',
                focusId: 'funcNcrN',
                ...
                build: () => CalculatorCommands.buildBinomialCoefficientExpression({
            ...
            fraction: {
                ...
                build: () => CalculatorCommands.buildFractionExpression({
            ...
            power: {
                ...
                build: () => CalculatorCommands.buildPowerExpression({
            ...
            integral: {
                ...
                build: () => CalculatorCommands.buildIntegralExpression({
    ...
    function updateStandardToolPreview() {
        ...
        try {
            const value = CalculatorUtils.evaluateExpression(definition.build());
            liveTarget.textContent = `Wert: ${CalculatorUtils.formatGeneralResult(value)}`;
        } catch {
    ...
    function applyFunctionBuilder(action) {
        const unaryBuilders = {
            ...
            'apply-func-exp': {
                ...
                build: (value) => CalculatorCommands.buildExpExpression(value),
            },
        };
        ...
        if (unaryBuilders[action]) {
            ...
            insertIntoMainInput(
                typeof build === 'function'
                    ...
                    : CalculatorCommands.buildUnaryFunctionExpression(name, byId(fieldId).value)
            );
            return;
        }
        ...
        if (action === 'apply-func-log') {
            ...
            insertIntoMainInput(
                CalculatorCommands.buildLogExpression({
                    base: byId('funcLogBase').value,
                    value: byId('funcLogValue').value,
                })
            );
        ...
        if (action === 'apply-func-factorial') {
            ...
            insertIntoMainInput(
                CalculatorCommands.buildFactorialExpression(byId('funcFactorialValue').value)
            );
            return;
        }
        if (action === 'apply-func-ncr') {
            ...
            insertIntoMainInput(
                CalculatorCommands.buildBinomialCoefficientExpression({
                    n: byId('funcNcrN').value,
                    k: byId('funcNcrK').value,
                })
            );
        ...
        if (action === 'apply-func-fraction') {
            ...
            insertIntoMainInput(
                CalculatorCommands.buildFractionExpression({
                    numerator: byId('funcFractionTop').value,
                    denominator: byId('funcFractionBottom').value,
                })
            );
        ...
        if (action === 'apply-func-power') {
            ...
            insertIntoMainInput(
                CalculatorCommands.buildPowerExpression({
                    base: byId('funcPowerBase').value,
                    exponent: byId('funcPowerExponent').value,
                })
            );
        ...
        if (action === 'apply-func-integral') {
            ...
            insertIntoMainInput(
                CalculatorCommands.buildIntegralExpression({
                    integrand: byId('funcIntegralExpression').value,
                    lowerBound: byId('funcIntegralLower').value,
                    upperBound: byId('funcIntegralUpper').value,
                })
    ...
    function adjustBinomValue(targetId, step) {
        ...
        if (!input) return;
        const raw = CalculatorUtils.normalizeNumberString(input.value || '0');
    ...
    function executeMainInput() {
        const mainInput = getMainInput();
        if (!mainInput) return;
        CalculatorCommands.execute(mainInput.value || '0', outputApi);
    ...
    function buildMatLiteralFromPanel(name) {
        ...
        return CalculatorCommands.buildMatLiteralString(rows);
    }
    ...
    function syncMatrixPreview() {
        ...
        try {
            const result = CalculatorCommands.evaluateMatrixExpr(expandedExpr);
            if (result.type === 'matrix') {
                liveResult.appendChild(CalculatorCommands.renderMatrixGrid(result.value));
            } else {
                liveResult.textContent = CalculatorUtils.formatGeneralResult(result.value);
            }
        } catch (e) {
