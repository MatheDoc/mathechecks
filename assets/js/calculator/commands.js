const CalculatorCommands = (() => {
    ...
    function parsePValue(pString) {
        const normalized = CalculatorUtils.normalizeNumberString(String(pString ?? ''));
    ...
    function getLiveBinomResult(fields) {
        const a = Math.ceil(parseFloat(CalculatorUtils.normalizeNumberString(fields.a || '0')));
        const b = Math.floor(parseFloat(CalculatorUtils.normalizeNumberString(fields.b || '0')));
        const n = parseInt(CalculatorUtils.normalizeNumberString(fields.n || '0'), 10);
        const p = parsePValue(fields.p || '0');
        const probability = CalculatorUtils.computeBinomProbability(a, b, n, p);
        if (!Number.isFinite(probability)) return '-';
        return CalculatorUtils.formatGeneralResult(probability);
    }
    ...
    function tryEvaluateLGSPanelExpression(rawValue) {
        const value = CalculatorUtils.evaluateWithAssignments(getLGSPanelExpression(rawValue), [], {});
    ...
    function evaluateExpressionWithX(rawExpression, xValue) {
        const normalized = CalculatorUtils.normalizeExpression(
    ...
    function evaluateScalarExpression(expr, varNames) {
        const value = CalculatorUtils.evaluateWithAssignments(expr, varNames, {});
        return Number.isFinite(value) ? value : 0;
    }

    function evaluateCoefficient(expr, varNames, targetVar) {
        const withOne = CalculatorUtils.evaluateWithAssignments(expr, varNames, { [targetVar]: 1 });
    ...
    function isLinearExpression(expr, varNames) {
        try {
            const eps = 1e-9;
            const baseline = CalculatorUtils.evaluateWithAssignments(expr, varNames, {});
            const coeffs = new Map();
            for (let i = 0; i < varNames.length; i++) {
                const variableName = varNames[i];
                const val1 = CalculatorUtils.evaluateWithAssignments(expr, varNames, { [variableName]: 1 });
                const val2 = CalculatorUtils.evaluateWithAssignments(expr, varNames, { [variableName]: 2 });
            ...
            for (let i = 0; i < varNames.length; i++) {
                for (let j = i + 1; j < varNames.length; j++) {
                    const vi = varNames[i];
                    const vj = varNames[j];
                    const both = CalculatorUtils.evaluateWithAssignments(expr, varNames, { [vi]: 1, [vj]: 1 });
    ...
    function solveLinearSystem(A, b) {
        ...
        for (let i = 0; i < pivotCols.length; i++) {
            const col = pivotCols[i];
            let expr = CalculatorUtils.formatGeneralResult(augmented[i][cols]);
            freeCols.forEach((freeCol) => {
                const coeff = -augmented[i][freeCol];
                if (Math.abs(coeff) < eps) return;
                const sign = coeff >= 0 ? ' + ' : ' - ';
                const absCoeff = Math.abs(coeff);
                const coeffText = absCoeff === 1 ? '' : CalculatorUtils.formatGeneralResult(absCoeff);
    ...
    function executeLGSCommand(input, outputApi) {
        try {
            ...
            outputApi.setText(
                solution.values.map((value, index) => `${varNames[index] || `x${index + 1}`}=${CalculatorUtils.formatGeneralResult(value)}`).join(', '),
                { headline: 'LGS' }
            );
        } catch (error) {
            outputApi.setText(
    ...
    function solveEquationCommand(equation, outputApi) {
        try {
            ...
            outputApi.setText(
                uniqueRoots
                    .map((root, index) => `${formatIndexedVariable('x', index + 1)}=${CalculatorUtils.formatGeneralResult(root)}`)
                    .join(', '),
                { headline: 'Gleichung' }
            );
        } catch {
    ...
    function expandExpression(input, outputApi) {
        try {
            const expr = CalculatorUtils.prepareGraphExpression(input);
    ...
    function buildGraphFunctionEvaluator(rawExpr) {
        const prepared = CalculatorUtils.prepareGraphExpression(rawExpr);
        const cache = new Map();
    ...
    function parseMatLiteral(str) {
        ...
        const rows = rowStrs.map((rowStr) => {
            ...
            return entriesRaw.map((e) => {
                const val = CalculatorUtils.evaluateExpression(e.trim());
    ...
    function parseMatrixAtom(text, matrixVars) {
        ...
        let scalarValue;
        try {
            scalarValue = CalculatorUtils.evaluateExpression(trimmed);
        } catch {
    ...
    function matSerialize(matrix) {
        return buildMatLiteralString(
            matrix.map((row) => row.map((v) => CalculatorUtils.formatGeneralResult(v)))
        );
    }
    ...
    function renderMatrixGrid(matrix) {
        ...
        matrix.forEach((row) => {
            row.forEach((val) => {
                ...
                cell.textContent = CalculatorUtils.formatGeneralResult(val);
    ...
    function executeMatrixCommand(input, outputApi) {
        try {
            ...
            if (result.type === 'matrix') {
                ...
                outputApi.setText(CalculatorUtils.formatGeneralResult(result.value), { headline: 'Ergebnis' });
            }
        } catch (e) {
    ...
    function execute(command, outputApi, { includeGraphAnalysis = true } = {}) {
        ...
        if (graphInner !== null) {
            ...
            const xMin = options.xmin ? parseFloat(CalculatorUtils.normalizeNumberString(options.xmin)) : -5;
            const xMax = options.xmax ? parseFloat(CalculatorUtils.normalizeNumberString(options.xmax)) : 5;
            const yMin = options.ymin ? parseFloat(CalculatorUtils.normalizeNumberString(options.ymin)) : null;
            const yMax = options.ymax ? parseFloat(CalculatorUtils.normalizeNumberString(options.ymax)) : null;
            const prepared = CalculatorUtils.prepareGraphExpression(func);
            if (!prepared) {
        ...
        if (equalsCount === 0 && containsVariableX(input)) {
            try {
                const value = CalculatorUtils.evaluateExpression(input);
                if (Number.isFinite(value)) {
                    outputApi.setText(CalculatorUtils.formatGeneralResult(value), { headline: 'Ergebnis' });
                    return;
                }
            } catch {
