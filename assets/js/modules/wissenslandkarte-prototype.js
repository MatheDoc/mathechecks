  ...
  const selectMarkup = withSelect
    ? `
      ...
      <select class="wl-select" id="assign-${escapeHtml(thought.id)}" data-thought-check="${escapeHtml(thought.id)}">
        ...
        ${checks
      .map(
        (check) => `<option value="${escapeHtml(check.check_id)}"${thought.checkId === check.check_id ? " selected" : ""
...
export async function initWissenslandkartePrototype({ root, lernbereich = "" } = {}) {
  if (!root) return;
  if (!lernbereich) {
    root.innerHTML = `<p class="module-status">Kein Lernbereich gesetzt.</p>`;
    return;
  }

  root.innerHTML = `<p class="module-status">Die Wissenslandkarte wird geladen...</p>`;

  try {
    const checks = await getChecksByLernbereich(lernbereich);
    if (!checks.length) {
      root.innerHTML = `<p class="module-status">Keine Checks für diesen Lernbereich gefunden.</p>`;
      return;
    }
    ...
    root.innerHTML = `<p class="module-status">Die Wissenslandkarte konnte nicht geladen werden.</p>`;
    console.error(error);
  }
}
