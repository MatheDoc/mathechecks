...
function renderCheckBadgeMarkup({ badgeClassName, badgeText, prefix, numberText }) {
  ...
  return `<span class="${escapeHtml(badgeClassName)}" aria-label="${escapeHtml(badgeText)}">${pre ? `<span class="check-card__badge-prefix">${escapeHtml(pre)}</span>` : ""
    }${nummer ? `<span class="check-card__badge-number">${escapeHtml(nummer)}</span>` : ""}</span>`;
}
...
export function renderCheckMetaRowMarkup({
  ...
  const rowClassName = normalizeClassName(["check-card__header-left", rowClass]);
  const toneBadgeClass = tone ? `check-card__badge--${tone}` : "";
  const toneTitleClass = tone ? `check-card__title--${tone}` : "";
  const badgeClassName = normalizeClassName(["check-card__badge", toneBadgeClass, badgeClass]);
  const titleClassName = normalizeClassName(["check-card__title", toneTitleClass, titleClass]);
