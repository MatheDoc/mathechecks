...
- Erzeugte Moodle-XMLs liegen unter `moodle/<gebiet>/<lernbereich>/`: eine pro Check (`<check_id>.xml`) sowie eine Sammeldatei für alle Checks des Lernbereichs (`alle_checks.xml`).
...
Konverter: `moodle/test_to_moodle.py` – liest alle JSON-Dateien und schreibt pro Check eine Moodle-XML nach `moodle/<gebiet>/<lernbereich>/<check_id>.xml` sowie pro Lernbereich `moodle/<gebiet>/<lernbereich>/alle_checks.xml`. Beide nutzen Fragetyp `multichoice`, `single=true`, `shuffleanswers=true`, Fragenname `<check_id>-NN` und Kategoriepfad `$course$/<gebiet>/<lernbereich>/<check_id>`. Die `fehler`-Einträge werden als falsch-spezifisches Antwort-Feedback übernommen. Vorlage für die XML-Parameter: `moodle/gebiet__lernbereich__01.xml`.
