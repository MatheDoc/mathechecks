-- Korrektur: Wiederholungsbadges richten sich nach jedem Aktivitätsabschluss,
-- nicht nach der fachlichen Quote. Auch ein Abschluss mit 0 % zählt.
...
begin
    ...
    with scored as (
        ...
        select check_id, max(created_at) as last_completed_at, count(*)::integer as completed_attempt_count
        from scored group by check_id
    ), check_quotes as (
        ...
        from windowed left join completion_history using (check_id)
    ...
    select jsonb_build_object(
        ...
        'checks', coalesce((select jsonb_agg(jsonb_build_object(
            ...
            'lastCompletedAt', last_completed_at, 'completedAttemptCount', completed_attempt_count,
