alter table public.session_activity_state
    drop constraint if exists session_activity_state_activity_type_check;

alter table public.session_activity_state
    add constraint session_activity_state_activity_type_check
    check (activity_type in ('flashcards', 'start'));
...
revoke all on function public.complete_start_activity(text) from public;
revoke all on function public.complete_start_activity(text) from anon;
grant execute on function public.complete_start_activity(text) to authenticated;
...
begin
    ...
    foreach current_lernbereich_slug in array normalized_lernbereiche loop
        ...
        on conflict (session_id, activity_key) do nothing;
