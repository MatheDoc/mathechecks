...
begin
    ...
    scope_tempo_days := greatest(coalesce(matched_tempo_days, 1), 1);

    select
        coalesce(card_state.level, 0),
        greatest(coalesce(source_session.tempo_days, scope_tempo_days, 1), 1)
    ...
    if not found then
        ...
        previous_level := 0;
        matched_tempo_days := scope_tempo_days;
    end if;
    ...
    if normalized_rating_key = 'hard' then
        ...
        interval_days := greatest(coalesce(matched_tempo_days, scope_tempo_days, 1), 1);
    else
        next_level := least(previous_level + 1, 8);
        interval_days := greatest(coalesce(matched_tempo_days, scope_tempo_days, 1), 1) * power(2, next_level)::integer;
    end if;
