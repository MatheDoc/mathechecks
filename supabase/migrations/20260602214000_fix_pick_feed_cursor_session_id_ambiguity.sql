-- Fix: In `pick_feed_cursor`, the RETURNS TABLE output column `session_id`
-- collides with unqualified `session_id` references inside SQL statements.
-- On the remote DB this shows up as `column reference "session_id" is ambiguous`
-- and aborts the cursor RPC before any current feed item can be selected.
