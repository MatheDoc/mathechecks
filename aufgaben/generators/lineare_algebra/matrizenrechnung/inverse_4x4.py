...
class Inverse4x4Generator(TaskGenerator):
    generator_key = "lineare_algebra.matrizenrechnung.inverse_4x4"
    ...
    def generate(self, count: int, seed: int | None = None) -> list[Task]:
        ...
        for _ in range(count):
            for _ in range(500):
                A = random_invertible(rng, 4, max_entry=9, steps_range=(6, 10), max_zeros=2)
                ...
                if any(abs(A_inv[i][j]) > 15 for i in range(4) for j in range(4)):
                    continue
                ...
                raise ValueError("Konnte keine weitere Aufgabe fuer Inverse 4x4 erzeugen.")
