"""Wahrscheinlichkeitsausdrücke im Sachzusammenhang interpretieren – mit bedingten Wkt.

Pro Aufgabe werden genau 6 Teilfragen gestellt (je eine aus):
  ...
  5. COND_A:      P(B|A), P(¬B|A), P(B|¬A), P(¬B|¬A)
  6. COND_B:      P(A|B), P(¬A|B), P(A|¬B), P(¬A|¬B)

Jede Teilfrage ist eine MC-Frage mit 24 Optionen (Moodle CLOZE-Format).
"""
...
# Feste Reihenfolge des MC-Pools (24 Optionen)
_POOL_KEYS_24 = (
    _GROUP_EINZEL
    + _GROUP_SCHNITT
    + _GROUP_VEREINIGUNG
...
def _build_mc_answer(question_key: str, scenario) -> str:
    """Baut den Moodle-MC-CLOZE-String mit 24 Optionen auf."""
    options = [_option_text(k, scenario) + "." for k in _POOL_KEYS_24]
    correct = _POOL_KEYS_24.index(question_key)
